...
void mainInstancing()
{
	...
	Shader shader_asteroid("asteroid.vert", "default_model.frag");

	// Load Model
	Model jupiter("models/airplane/scene.gltf");
	...
	for (unsigned int i = 0; i < number; i++)
	{
		...
		float x = randf();
		float final_radius = radius + randf() * radius_deviation;
		...
		if (randf() > 0.5f)
		{
			...
			temp_translation = glm::vec3(y * final_radius, randf(), x * final_radius);
		}
		else
		{
			// Generates a translation near a circle of radius "radius"
			temp_translation = glm::vec3(x * final_radius, randf(), y * final_radius);
		}
		// Generates random rotations
		temp_rotation = glm::quat(1.0f, randf(), randf(), randf());
		// Generates random scales
		temp_scale = 0.1f * glm::vec3(randf(), randf(), randf());
		...
		trans = glm::translate(trans, temp_translation);
		rot = glm::mat4_cast(temp_rotation);
		sca = glm::scale(sca, temp_scale);

		// Push matrix transformation
		instance_matrix.push_back(trans * rot * sca);
	}
	// Create the asteroid model with instancing enabled
	Model asteroid("models/asteroid/scene.gltf", number, instance_matrix);
	...
	shader_asteroid.useProgram();
	glUniform4f(glGetUniformLocation(shader_asteroid.m_id, "u_light_color"), light_color.x, light_color.y, light_color.z, light_color.w);
	glUniform3f(glGetUniformLocation(shader_asteroid.m_id, "u_light_position"), light_position.x, light_position.y, light_position.z);
	...
	{
		"skybox/space/right.png","skybox/space/left.png","skybox/space/top.png","skybox/space/bottom.png","skybox/space/front.png","skybox/space/back.png"
	};
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		jupiter.drawModel(shader_default, camera);
		asteroid.drawModel(shader_asteroid, camera);
