...
vec4 pointLight(float a, float b)
{
	...
	float diffuse = max(abs(dot(normal, light_direction)), 0.0f); //Grass Shading
	...
	if(texture(u_diffuse0, io_texture_coordinator).a < 0.1)
		discard;
...
vec4 directLight()
{
	...
	float diffuse = max(abs(dot(normal, light_direction)), 0.0f);
